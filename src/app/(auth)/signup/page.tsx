...
import { requestSignupOtp, verifySignupOtp, completeSignup } from "@/lib/actions/auth";
import { StepDots } from "@/components/step-dots";

type Step = "details" | "otp" | "password";
const STEP_INDEX: Record<Step, number> = { details: 0, otp: 1, password: 2 };
...
export default function SignupPage() {
  ...
  async function handleDetailsSubmit(e: FormEvent) {
    ...
    setError(null);
    setLoading(true);
    const result = await requestSignupOtp({ name, phone });
    setLoading(false);
    if (!result.ok) {
  ...
  async function handlePasswordSubmit(e: FormEvent) {
    ...
    setError(null);
    setLoading(true);
    const result = await completeSignup({ name, phone, password, confirmPassword });
    setLoading(false);
    if (!result.ok) {
      setError(result.error);
    }
